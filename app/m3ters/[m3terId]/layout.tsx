...
const navItems = [
  { name: "Charts", icon: <FaChartLine />, path: "charts" },
  ...
  return (
    <Link href={item.path} title={item.name}>
      <motion.div
        ...
        className={`flex items-center justify-start px-3 py-3 my-3 rounded-2xl group ${
...
const DashboardLayout = ({ children }: PropsWithChildren) => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen">
      ...
      {/* Main content */}
      <main className="flex-1 p-10 md:pb-0">{children}</main>
